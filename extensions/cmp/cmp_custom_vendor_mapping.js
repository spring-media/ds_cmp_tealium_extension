...
const processUtag = function (str, shouldFireView) {
    if (!window.__utag_view_fired) {
        ...
        if (((existingCookie && existingCookie[0].indexOf('kameleoon') >= 0) || 
            ...
            utag.data['cp.hasPurSubscription'] && utag.data['cp.hasPurSubscription'] === 'false' && 
            utag.data.user_hasPurSubscription && utag.data.user_hasPurSubscription == 'false') 
            {
                window.utag.view(utag.data, null, getDomainTagValue(window.location.hostname, 'kameleoon'));
            }
    }
};
