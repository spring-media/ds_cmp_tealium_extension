    ...
    window.cookieRead =
        ...
        function (c) {
            ...
            return (c =
                0 > a
                    ? ''
                    : decodeURIComponent(b.substring(a + 2 + c.length, 0 > d ? b.length : d)))
                ? c
                : '';
        };
    var l;
...
s._utils = {
    isDocTypeArticle: function() {
        const docType =
            window.utag.data.mapped_page_document_type ||
...
s._setPageAgeForCheckout = function() {
    if (typeof window.utag.data.page_age !== 'undefined' && s._utils.isDocTypeArticle()) {
...
s._bildAppsPageNameObj = {
    isLive: function() {
        return (
            ...
            (window.utag.data.page_cms_path.indexOf('/im-live-ticker/') !== -1 ||
                ...
                (!!window.utag.data.keywords &&
                    window.utag.data.keywords.indexOf('Live-Ticker') !== -1))
        );
    },
    ...
    isSport: function() {
        ...
        return (
            !!window.utag.data.page_cms_path &&
            (lowerCmsPath.indexOf('sport/') !== -1 || lowerCmsPath.indexOf('sportdaten') !== -1)
        );
    },
...
s._setPageCmsPathWithoutBild = function(s) {
    if (
        typeof window.utag.data.page_cms_path !== 'undefined' &&
...
s._prevPageObj = {
    ...
    isFromHomePageId: function(s) {
        return (
            s._prevPage.includes('20595788') ||
            s._prevPage.includes('52081556') ||
            s._prevPage.includes('26324062') ||
            s._prevPage.includes(
                '52081598' ||
                    s.pageName.includes('jJt31TLVzDZzInKWHyk1') ||
                    s.pageName.includes('XqNqOGKoIKo1DfG74jWr')
            )
        );
    },
    ...
    isHomePage: function(s) {
        return (
            s.pageName.includes('20595788') ||
            s.pageName.includes('52081556') ||
            s.pageName.includes('26324062') ||
            s.pageName.includes('52081598') ||
            s.pageName.includes('jJt31TLVzDZzInKWHyk1') ||
            s.pageName.includes('XqNqOGKoIKo1DfG74jWr')
        );
    },
